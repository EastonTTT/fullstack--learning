...
export const ValList = () => {
    ...
    const handleClick = async () => {
        try {
            const newValArr = await updateValList([...valArr,{id: valArr.length, content: newNote}]);
            setValArr(newValArr)
            setNewNote('')
        } catch (error) {
            console.log(error)
    ...
    return (
        <div className={styles.container}>
            <List valList={valArr}/>
            <input type="text" value={newNote} onChange={handleInputChange} placeholder="please type here.."/>
